...
async def _main(is_simulating: bool, current: float) -> None:
    ...
    CUSTOM_AXIS_SETTINGS[types.OT3Axis.Z_L].run_current = current
    ...
    await api.move_rel(types.OT3Mount.LEFT, types.Point(z=-50), speed=10)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--simulate", action="store_true")
    parser.add_argument("--z-amps", type=float, default=1.0)
    args = parser.parse_args()
    print(f'Current={args.z_amps}')
    asyncio.run(_main(args.simulate, args.z_amps))
